...
PORT_REMOTE1=7000
PORT_REMOTE2=7001
TOTAL_KEYS=7500           # Total keys to insert (~5k will land on remote nodes)
...
declare -a remote_insert_times
remote_keys_inserted=0
local_keys_inserted=0

log "=== Starting Key Insertion (Target: $TOTAL_KEYS keys) ==="
...
for ((i=1; i<=$TOTAL_KEYS; i++)); do
  ...
  if (( slot >= remote1_slot_start && slot <= remote1_slot_end )); then
    # Time remote1 insertion
    start_time=$(date +%s.%N)
    result=$(redis-cli -c -h $HOST_REMOTE -p $PORT_REMOTE1 SET "$key" "$value")
    elapsed=$(echo "$(date +%s.%N) - $start_time" | bc)
    remote_insert_times+=("$elapsed")
    ((remote_keys_inserted++))
    node="remote1"
  elif (( slot >= remote2_slot_start && slot <= remote2_slot_end )); then
    # Time remote2 insertion
    start_time=$(date +%s.%N)
  ...
  # Progress tracking
  if (( i % 500 == 0 )); then
    log "Inserted $i/$TOTAL_KEYS keys... (Remote: $remote_keys_inserted, Local: $local_keys_inserted)"
  fi
done

total_time=$(echo "$(date +%s.%N) - $total_start_time" | bc)

# Calculate remote insertion statistics
remote_total_time=0
remote_min_time=0
remote_max_time=0
remote_avg_time=0

if (( ${#remote_insert_times[@]} > 0 )); then
  remote_min_time=${remote_insert_times[0]}
  remote_max_time=${remote_insert_times[0]}

  for time in "${remote_insert_times[@]}"; do
    remote_total_time=$(echo "$remote_total_time + $time" | bc)

    if (( $(echo "$time < $remote_min_time" | bc -l) )); then
      remote_min_time=$time
    fi

    if (( $(echo "$time > $remote_max_time" | bc -l) )); then
      remote_max_time=$time
    fi
  done

  remote_avg_time=$(echo "scale=6; $remote_total_time / ${#remote_insert_times[@]}" | bc)
fi
...
log "--- Remote Insertion Metrics ---"
log "Total remote insertion time: $remote_total_time seconds"
log "Average remote insertion time: $remote_avg_time seconds"
log "Minimum remote insertion time: $remote_min_time seconds"
log "Maximum remote insertion time: $remote_max_time seconds"
